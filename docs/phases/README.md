...
Every sub-phase (1.1, 1.2, 1.3, etc.) gets a **phase-completion document** once it's verified working. This is not a running log (that's `changelog.md`) or a reference (that's `knowledge.md`) — it's a **snapshot** of what was built, how it connects, and what it looks like at the moment the sub-phase was declared done.
...
3. Add section-by-section mermaid diagrams showing how files connect (like the "How Everything Connects" section in `knowledge.md`)
...
| Architecture (mermaid) | One high-level "what was built" diagram. Detailed diagrams live in `knowledge.md` — don't duplicate. | You write this |
...
docs/knowledge.md                   ← reference for ALL concepts/commands/architecture diagrams
...
**No duplication.** The phase doc has a single high-level diagram + a pointer to `knowledge.md` for detailed architecture. `knowledge.md` is the permanent reference for how things connect. The phase doc is the historical snapshot of what happened.
