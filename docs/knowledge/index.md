...
| [dlt.md](dlt.md) | dlt (data load tool) | **Why we switched from Airbyte to dlt** (WSL2 resource constraints: 5-6 containers + 2-4GB RAM vs 1 pip install + ~100MB), dlt vs Airbyte decision matrix, what we did NOT lose by switching, dlt concepts, installation, BigQuery config, append vs replace, schema evolution, our Phase 4.4 usage (34.8M Divvy trips from S3), gotchas |
| [bigquery-ml.md](bigquery-ml.md) | BigQuery ML | BQML concepts (CREATE MODEL, ML.EVALUATE, ML.WEIGHTS, ML.PREDICT), dbt integration via post_hook, gotchas (categorical weight NULL, high-cardinality unseen categories, no_split + ML.EVALUATE error), our Phase 4.8 usage (linear_reg on crime vs ridership, R²=0.434 in-sample, crime coefficient +1.45) |
