...
`dbt docs generate` introspects the warehouse and builds a `catalog.json` (column types, stats, table metadata). Combined with `manifest.json` (generated by every `dbt build`), this powers an interactive HTML site showing every model, source, test, and macro with its description, columns, dependencies, and compiled SQL.

```bash
# Generate (run after dbt build so manifest.json is fresh)
dbt docs generate --project-dir /opt/airflow/dbt --profiles-dir /opt/airflow/dbt_profiles

# Serve (starts a local web server)
dbt docs serve --project-dir /opt/airflow/dbt --profiles-dir /opt/airflow/dbt_profiles --port 8090
```

**Port:** 8090 (8080 conflicts with Airflow). The `--host 0.0.0.0` flag is needed inside Docker so the port is accessible from the host.

**In our setup:** dbt runs in a separate container (`chicago-data-pipeline-dbt:latest`), so `dbt docs serve` runs as a Docker container with `-p 8090:8090`. The docs artifacts (`catalog.json`, `manifest.json`) live in `dbt/target/` which is mounted via `--volumes-from` the Airflow scheduler container.

**What you see:** 15 models (staging + marts + BQML), 4 sources, 89 data tests, 1 seed, 871 macros. Each model page shows: description (from `schema.yml`), columns with types + descriptions, compiled SQL, upstream/downstream dependencies, and test results.

**BigQuery gotcha:** `dbt docs generate` against BigQuery shows a harmless `RuntimeWarning: "table_owner" does not match the name of any column` — BigQuery doesn't have table owners like Postgres. The catalog is still built correctly.

---

**← Previous:** [airflow](airflow.md) | **Next:** [grafana](grafana.md) →
