...
Reference for Terraform concepts, workflow, our setup, and errors hit during Phase 4.2. Terraform provisions cloud infrastructure (BigQuery datasets + GCS bucket) as code — see `docs/wiki/gcp.md` for the GCP auth model that Terraform builds on.
...
### Providers
A provider is a plugin that knows how to talk to a specific cloud (Google, AWS, Azure). We use `hashicorp/google` (v7.40.0, pinned `~> 7.40`). The provider config includes how to authenticate — in our case, via a service account key file path. See `docs/wiki/gcp.md` for the auth model.
...
The key file is the same one created in Phase 4.1 (`docs/wiki/gcp.md` Step 7). Terraform authenticates as the `terraform-runner` service account with scoped roles (bigquery.dataOwner, storage.admin, etc.) — least privilege. See `docs/wiki/gcp.md` for the full auth model.
...
**Lesson:** `gcloud auth activate-service-account` is the non-interactive auth path — use it in WSL (no browser) and in CI/CD. `gcloud auth login` (browser) is for humans only. See `docs/wiki/gcp.md` pitfall #5.
...
**Lesson:** Any gcloud command taking a file path needs the full path, not `~`. See `docs/wiki/gcp.md` pitfall #1.
...
**← Previous:** [gcp](gcp.md) | **Next:** [dlt](dlt.md) →
